...
function mrkapowski_attachment_attr( $attr, $attachment, $size ) {
	if ( isset( $attr['class'] ) && ( strpos($attr['class'], 'custom-logo') === false || !is_singular()) ) {
...
function saorsa_add_kind_types() {
	register_post_kind(
		'follow-of',
			array(
				...
				'property'        => 'follow-of', // microformats 2 property
							...
							'description'     => __( 'a "follow" of another site, feed, or online persona', 'indieweb-post-kinds' ),
							'description-url' => 'http://indieweb.org/follow',
							'title'           => false, // Should this kind have an explicit title
							'show'            => true, // Show in Settings
			)
	);
}
add_filter( 'init', 'saorsa_add_kind_types' );
function saorsa_extra_site_icons( $return, $url) {
	$sites = array(
		'mastodon.xyz'   => 'mastodon',
		'indieweb.xyz'   => 'website',
	);
...
//add_filter( 'syn_link_mapping', 'saorsa_extra_site_icons', 10, 2);
function saorsa_extra_silo_icons( $silos) {
	$new_silos = array(
		'mastodonxyz' => array(
			'baseurl' => 'https://mastodon.xyz/%s',
			'display' => __( 'Mastodon.xyz username', 'indieweb' ),
		),
	);
	return array_merge($silos, $new_silos);
}
//add_filter( 'wp_relme_silos', 'saorsa_extra_silo_icons', 10, 2);
function saorsa_url_to_name( $website, $url ) {
	$strings = array_keys( simpleicons_iw_get_names() );
	if ( false !== stripos( $url, 'mastodon.' ) ) {
		return 'mastodon';
	}
...
//add_filter( 'indieweb_links_url_to_name', 'saorsa_url_to_name', 10, 2);
